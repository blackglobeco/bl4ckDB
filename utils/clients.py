        ...
        while retry_count < max_retries:
            try:
                ...
                if "FLOOD_WAIT" in error_msg:
                    import re
                    wait_time = re.search(r'wait (\d+) seconds', error_msg)
                    if wait_time:
                        wait_seconds = int(wait_time.group(1))
                        logger.warning(
                            f"Flood wait detected for {type.title()} Client {client_id}. "
                            f"Waiting {wait_seconds} seconds before retry..."
                        )
                        await asyncio.sleep(wait_seconds + 5)  # Add 5 extra seconds as buffer
                        retry_count += 1
                        continue
                ...
                break  # Non-flood error, exit retry loop
